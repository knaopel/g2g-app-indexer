							...
							($PageList | ? { $_.PageId -eq $PageId }).WebPartId | Get-Unique | % `
							{
								...
								if (ShouldIndex-WebPart $WpConfig) {
									...
									if ($WpConfig -match 'app-class="G2G.Apps.Faq') {
										...
										$Keywords = Get-ListKeywords -Site $Site -ListGuid $RefList -ViewFields $ViewFields -RenderTemplate '"<div>{0}, {1}</div>`n" -f $_.Title, $_.Answer'
										$App = "FAQ"
									}
									...
									if ($WpConfig -match 'app-class="G2G.Apps.ContactInfo') {
										...
										$Keywords = Get-ListKeywords -Site $Site -ListGuid $RefList -ViewFields $ViewFields -RenderTemplate 'Render-ListData $_ -Columns $Columns'
										$App = "Contact Info"
									}

									if ($WpConfig -match 'app-class="G2G.Apps.FilterList') {
										$Columns = Get-ConfigColumns $WpConfig -Regex "<div app-attr=`"Columns`">(.*?)</div>";
										#Add-Content $LogFile "App: FilterList`r`nPage: $_`r`nAppConfig: $WpConfig"
										...
										$ViewFields = Get-ViewFieldsForColumns $Columns;
										$Keywords = Get-ListKeywords -Site $Site -ListGuid $RefList -CamlQuery $CamlQuery -Suffix $Suffix -ViewFields $ViewFields -RenderTemplate 'Render-ListData $_ -Columns $Columns -Mappings $Mappings'
										$App = "Filter List"
									}
									...
									if ($WpConfig -match 'app-class="G2G.Apps.ContentSection') {
										...
										#Add-Content $LogFile "App: ContentSection`r`nPage: $_`r`nAppConfig: $WpConfig"
										$Mappings = Get-ConfigMappings $WpConfig -Regex "<div app-attr=`"IndexedColumnMappings`">(.*?)</div>";
										$CamlQuery = Get-ListDataSourceCamlQuery $WpConfig;
										$ViewFields = Get-ViewFieldsForColumns $Columns;
										$Keywords = Get-ListKeywords -Site $Site -ListGuid $RefList -CamlQuery $CamlQuery -ViewFields $ViewFields -RenderTemplate 'Render-ListData $_ -Columns $Columns -Mappings $Mappings'
										$App = "Content Section"
									}
									...
									##
									Set-LibraryVersioning -PagesLibrary $Page.Item.ParentList -Status "Off"
...
#									}
				...
				## ensure versioning is back on for Library
				Set-LibraryVersioning -PagesLibrary $Web.Lists.TryGetList("Pages") -Status "On"
			}
		}
	}
}
...
function Set-LibraryVersioning {
	param(
	[Parameter(Mandatory = $true)]
	[Microsoft.SharePoint.SPList]$PagesLibrary,
	[Parameter(Mandatory = $true)]
	[ValidateSet("On","Off")]$Status
	)
	switch ($Status) {
		"Off" {
			if ($PagesLibrary.EnableVersioning) {
				$PagesLibrary.EnableVersioning = $false;
				$PagesLibrary.ForceCheckout = $false;
				$PagesLibrary.Update();
			}
		}
		"On" {
			if (-not $PagesLibrary.EnableVersioning) {
				$PagesLibrary.ForceCheckout = $true;
				$PagesLibrary.EnableVersioning = $true;
				$PagesLibrary.EnableMinorVersions = $true;
				$PagesLibrary.MajorVersionLimit = 0;
				$PagesLibrary.MajorWithMinorVersionsLimit = 0;
				$PagesLibrary.DraftVersionVisibility = [Microsoft.SharePoint.DraftVisibilityType]::Author
				$PagesLibrary.Update();
			}
		}
	}
...
function Get-ListKeywords {
	...
	catch 
	{
		if ($_.Exception -like '*does not exist*') {
			...
			$error | Out-Host
			ac $LogFile  $error
			Add-Content $LogFile "ERR: $($_.Exception)"
		} else {
			$_.Exception | Out-Host
			ac $LogFile  $_.Exception
		}
...
function Get-ConfigColumns {
	...
	$Columns = @();
	$Config | Select-String -Pattern $Regex -AllMatches | %{$Columns = $_.Matches.Groups[1].Value.Split(",") }
	return $Columns;
}
...
function Update-G2GAppIndex {
	...
	Add-Content -Path $LogFile -Value "Run by $($ENV:USERNAME).`r`n";
	Add-Content -Path $LogFile -Value "Page, App, WebPartId";
	
	Get-SPWebApplication -Identity $WebApplicationUrl | Get-SPSite -Limit ALL | % {
		$DBName = $_.ContentDatabase.Name;
		$SiteId = $_.Id;
		
		$PageList = Get-PagesList -DBName $DBName -SiteId $SiteId -SQLInstance $SQLInstance
		if ($PageList -ne $null) {
...
Export-ModuleMember -Function "Update-G2GAppIndex"